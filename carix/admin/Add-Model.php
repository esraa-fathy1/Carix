...
if(isset($_GET['update'])){
    $mode = 'update';
    $model_id = $_GET['update'];
    $select_model = "SELECT * FROM `models` 
    WHERE `id` = $model_id";
    $run_select_model = mysqli_query($connect , $select_model);
    $array = mysqli_fetch_array($run_select_model);
}
...
if(isset($_POST['add'])){
    $name = $_POST['name'];
    $brand_id =  $_POST['brand'];
    $insert = "INSERT INTO `models` VALUES(NULL , '$name' , $brand_id)";
                ...
                <div class="inner">
                    ...
                    <?php if($mode == 'add'){?>
                        ...
                        <div class="form-holder">
                            <input type="text" placeholder = "Model Name" class="form-control" style="font-size: 15px;"  value="<?php if($mode == 'update'){ echo $array['name'] ;} ?> " name="name" required>
                        </div>
                        <div class="form-holder">
                            <select id="type" name="brand" class="form-select" required>
                                <?php foreach($run_select_brand as $brand){?>
                                <option value="<?php echo $brand['id'] ;?>" <?php if($mode == 'update' && $brand['id'] == $array['brand_id']) { ?> selected <?php } ?> ><?php echo $brand['name'] ;?></option>
                                <?php } ?>
                            </select>
                        </div>
