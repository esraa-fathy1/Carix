...
if (isset($_POST['verify'])) {
    ...
    $real_otp = $_SESSION['verificationCode'];
    
    if ($code == $real_otp) {
        $welcome = "welcome to our website";

        // Send the verification code to the user's email
        $mail->setFrom('carix2024@gmail.com', 'Carix'); // h8yr henaaa
        $mail->addAddress($email);
        $mail->isHTML(true);
        $mail->Subject = 'Account Registration Verification';
        $mail->Body = "Hi Mr/Mrs $name,': $welcome";
        ...
        if (!$mail->send()) {
            ...
            $code='carix'.$code;
        $insert = "INSERT INTO `users` values(NULL,'$name',NULL,'$email','$password','$code')";
        $run_insert = mysqli_query($connect, $insert);

        $select_id="SELECT `id` FROM `users` WHERE `email`='$email'";
        $run_select_id=mysqli_query($connect,$select_id);
        $row=mysqli_fetch_array($run_select_id);
        ...
        //insert wallet
        $insert_wallet="INSERT INTO `wallets` values(NULL,'0','$row[id]')";
        $run_insert_wallet=mysqli_query($connect,$insert_wallet);
    ...
    <section class="mainBg py-5">
        <div class="container multiSteps">
            <div class="row align-items-center">
                <div class="col-6">
                    <img src="../images/Forms/Verify.svg" alt="Form Image" class="img-fluid">
                </div>
                <div class="col-6 selectForm usualForm pe-2">
                    <form id="form1" method="POST">
                        ...
                        <div class="container otp">
                            <div id="inputs" class="inputs d-flex gap-4">
                                <input class="input" type="text" inputmode="numeric" name="d1" maxlength="1" />
                                <input class="input" type="text" inputmode="numeric" name="d2" maxlength="1" />
                                <input class="input" type="text" inputmode="numeric" name="d3" maxlength="1" />
                                <input class="input" type="text" inputmode="numeric" name="d4" maxlength="1" />
                                <input class="input" type="text" inputmode="numeric" name="d5" maxlength="1" />
                                <input class="input" type="text" inputmode="numeric" name="d6" maxlength="1" />
                            </div>
                        </div>
                        <div class="btn-box mt-4">
                            <button type="submit" id="verfy" name="verify">Verify</button>
                        </div>
                        <?php if($alert){ ?>
