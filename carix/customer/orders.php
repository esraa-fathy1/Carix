...
if(isset($_POST['ok'])) {
    ...
    if($text=="1"){
        ...
        JOIN `user_address` ON `orders`.`address_id`=`user_address`.`add_id`
        JOIN `users` ON `orders`.`user_id`=`users`.`id`
        WHERE `orders`.`user_id`='$user_id' ORDER BY `o_id` ASC ";
        ...
        JOIN `user_address` ON `orders`.`address_id`=`user_address`.`add_id`
        JOIN `users` ON `orders`.`user_id`=`users`.`id`
        WHERE `orders`.`user_id`='$user_id'  ORDER BY `o_id` DESC";
        $run_select_orders=mysqli_query($connect,$select_orders);
    }
                ...
                <div class="inner">
                    ...
                    <form method="POST">
                        ...
                        <div class="justify-content-between flex-row d-flex">
                            <select id="brand" name="sort" aria-label="Sort By" class="form-select w-75">
                                <option  value="1">Sort By: Oldest</option>
                                <option  value="2">Sort By: Newest</option>
                            </select>
                            <button type="submit" class="sort rounded" name="ok">OK</button>
                        </div>
                       
                        <!-- <div class="border p-3 mt-3">
                            <div class="d-flex gap-2 flex-column">
                                <p class="d-flex justify-content-between fw-bold">
                                    <span>Arrives By June 24, 2024</span>
                                </p>
                                <p class="d-flex justify-content-between">
                                    <span>To: Heliopolis, Cairo</span>
                                </p>
                                <p class="d-flex justify-content-between fw-bold">
                                ...
                                <p class="d-flex justify-content-between fw-bold wallet">
                                    <span>Oil Change 10,000 (Shell)</span>
                                    <span>1,950</span>
                                </p>
                                <p class="d-flex justify-content-between fw-bold wallet">
                                    <span>Oil Change 5,000 (Shell)</span>
                                    <span>1,070</span>
                                </p>
                            </div>
                            <hr>
                            <p class="d-flex justify-content-between text-dark-gray mb-4 fs-5 fw-bold">
                                <span>Total</span>
                                <span class="total">3,220 EGP</span>
                            </p>
                            <p class="d-flex justify-content-center mt-3 fw-bold status">Status: Cancelled</p>
                        </div> -->
                       
                         <?php foreach($run_select_orders as  $row){ 
                            $order_id=$row['o_id']; 
                            // echo $order_id;
                            $select_details="SELECT * FROM `order_details` 
                            JOIN `orders` ON `order_details`.`order_id`=`orders`.`o_id`
                            LEFT JOIN `users` ON `order_details`.`user_id`=`users`.`id`
                            LEFT JOIN `products` ON `order_details`.`product_id`=`products`.`p_id`
                            LEFT JOIN `services` ON `order_details`.`service_id`=`services`.`s_id`
                            WHERE `order_details`.`order_id`='$order_id'";
                            $run_select_details=mysqli_query($connect,$select_details);
                            $row_select_details=mysqli_fetch_assoc($run_select_details);
...
?>
                        ...
                        <div class="border p-3 mt-3">
                            <div class="d-flex gap-2 flex-column">
                                <p class="d-flex justify-content-between fw-bold">
                                    <span>Arrives By <?php echo $row['date'] ?></span>
                                </p>
                                <p class="d-flex justify-content-between">
                                    <span>To: <?php echo $row['address'] ?></span>
                                </p>
                                <p class="d-flex justify-content-between fw-bold">
                                    <span>Order Number</span>
                                ...
                                <?php foreach($run_select_details as $data ){ ?>
                                <p class="d-flex justify-content-between fw-bold wallet">
                                    <?php if($data['p_id']!= NULL) {?>
                                    <span><?php echo $data['p_name'] ?></span>
                                    <span><?php echo $data['p_price'] ?></span>
                                    <?php } ?>
                                </p>
                                <?php  if($data['s_id']!= NULL) {?> 
                                <p class="d-flex justify-content-between fw-bold wallet">
                                <span><?php echo $data['s_name'] ?></span>
                                    <span><?php echo $data['s_price'] ?></span>
                                    <?php } } ?>
                                </p>
                            ...
                            <p class="d-flex justify-content-center mt-3 fw-bold status">Status: <?php echo $row['status'] ?></p>
                        </div>
                        <?php }  ?>
